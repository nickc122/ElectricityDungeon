...
var in_hazard: bool = false #whether currently in a damaging hazard
...
func _on_hitbox_area_area_entered(area: Area2D) -> void: #areas on a physics layer called Hazards
	...
	var hazard_timer = area.hazard_timer #receives the time for how often to damage
	take_damage(damage) #runs the damage function for the first time
	in_hazard = true #sets that player is now in a hazard
...
func _on_hitbox_area_area_exited(area: Area2D) -> void:
	in_hazard = false #detects that player left area
	
func _on_hazard_timer_timeout() -> void:
	if in_hazard: take_damage(damage) #when the timer finishes damages player again if still in area
